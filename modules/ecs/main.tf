...
======*/
resource "aws_cloudwatch_log_group" "booksapi" {
  name = "booksapi"
...
======*/
resource "aws_ecr_repository" "booksapi_app" {
...
data "template_file" "web_task" {
  ...
  vars {
    image           = "${aws_ecr_repository.booksapi_app.repository_url}"
    log_group       = "${aws_cloudwatch_log_group.booksapi.name}"
  }
}
...
resource "aws_alb" "alb_booksapi" {
  name            = "${var.environment}-alb-booksapi"
...
resource "aws_alb_listener" "booksapi" {
  load_balancer_arn = "${aws_alb.alb_booksapi.arn}"
  port              = "80"
  protocol          = "HTTP"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = "${var.environment}_booksapi_web_cpu_utilization_high"
